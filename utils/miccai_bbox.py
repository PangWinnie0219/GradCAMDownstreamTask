...
def GradCAM_model(train_dataset, train_num_each, val_dataset, val_num_each):
    """plot and save the image of interest (define by bidx):
    1. image with predicted bounding box
    2. image with GT bounding box
    3. Grad-CAM heatmap of class of interest (define by tclass)
    ...
    for idx, data in enumerate(val_loader):
        ...
        if idx >= frame_no:
            vid_name = "seq_" + video_list[vid_no]
        ...
        ## to view the heatmap of specific image, define by bidx
        if idx >= args.bidx:
            input_tensor = inputs  # Normalized image
            rgb_img = inputs_wo_nor[0]  # RGB image
            ...
            print("pred:", torch.sigmoid(outputs_tool[0]).detach().cpu().numpy())
            print(
                "score:",
                torch.round(torch.sigmoid(outputs_tool[0])).detach().cpu().numpy(),
            )
            print("label:", labels_tool[0].detach().cpu().numpy())
            ...
            plot_gtbox(args, rgb_img, gt_bbox_frame)

            """------------------- Grad-CAM heatmap Plotting ---------------------------------"""

            # If target_category = None, returns the map for the highest scoring category.
