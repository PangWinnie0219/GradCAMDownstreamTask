...
class BaseCAM:
    ...
    def get_cam_image(
        ...
        )  # shape of weights: (81, 2048)
        weighted_activations = (
            weights[:, :, None, None] * activations
        )  # shape of weighted_activations: 81, 2048, 4, 7
        if eigen_smooth:
    ...
    def forward(
        self, input_tensor, target_category=None, eigen_smooth=False, target_size=None
    ):
        if self.cuda:
    ...
    def compute_cam_per_layer(
        self, input_tensor, target_category, eigen_smooth, target_size
    ):
        activations_list = [
        ...
        if target_size == None:
            target_size = self.get_target_width_height(
                input_tensor
            )  # (w, h): (210, 120)
        ...
        for target_layer, layer_activations, layer_grads in zip(
            ...
            cam = self.get_cam_image(
                input_tensor,  # print(cam.shape): (81, 4, 7)  cam=>numpy array
                target_layer,
                target_category,
                layer_activations,
                layer_grads,
            ...
            #     cam[cam<0.3]=0
            # elif target_category == [10]:
            #     cam[cam<0.1]=0

            cam[
                cam < 0.0
            ] = 0  # works like mute the min-max scale in the function of scale_cam_image  # make those <0 = 0 range[0,1.7495756]
            # print(np.min(cam[0]), np.max(cam[0]))

            scaled = self.scale_cam_image(
                cam, target_size
            )  # from size (81, 4, 7) to (81, 120, 210) , value ranged from [0,1.7495756] to [0.0 0.99988055]
            # print(np.min(cam[0]), np.max(cam[0]), np.min(scaled[0]), np.max(scaled[0]))
            cam_per_target_layer.append(
                scaled[:, None, :]
            )  # (81, 1, 120, 210)  (bs,num_target_layer,h,w)
    ...
    def __call__(
        ...
        return self.forward(input_tensor, target_category, eigen_smooth, target_size)
...
def show_cam_on_image(
    ...
    heatmap = cv2.applyColorMap(
        np.uint8(255 * mask), colormap
    )  # heatmap.shape: (120, 210, 3)
