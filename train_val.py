...
def train(args, epoch, num_train_all, model, train_loader, optimizer, criterion):
    ...
    for idx, data in enumerate(train_loader):
        ...
        # else:
        #     inputs, labels_tool = data[0], data[1]
        ...
        if batch_progress*args.bs >= num_train_all:
            ...
            print('Batch progress: %s [%d/%d]' % (str(percent) + '%', batch_progress*args.bs, num_train_all), end='\r')
...
def val(args, epoch, num_val_all, model, val_loader, criterion):
    ...
    with torch.no_grad():
        for data in val_loader:
            ...
            if val_progress*args.bs >= num_val_all:
                ...
                print('Val progress: %s [%d/%d]' % (str(percent) + '%', val_progress*args.bs, num_val_all), end='\r')
    ...
    val_acc = accuracy_score(np.array(val_labels_tool_list), np.array(val_scores_tool_list))
